...
describe('The PrimeSDK, when transfer a token with fuse network on the MainNet', function () {
  it('SMOKE: Validate the time taken for the regular ERC20 token transaction on the fuse network', async function () {
    ...
    await customRetryAsync(async function () {
      ...
      let op;
      try {
        op = await fuseMainNetSdk.estimate();
      } catch (e) {
        console.error(e);
        const eString = e.toString();
        addContext(test, eString);
  ...
  it('SMOKE: Validate the time taken for the sponsored transaction on the fuse network', async function () {
    var test = this;
  ...
  it('SMOKE: Validate the time taken for the gasless ERC20 token transaction on the fuse network', async function () {
    var test = this;
    let arka_url = data.paymaster_arka;
    let queryString = `?apiKey=${process.env.API_KEY}&chainId=${Number(
      data.fuse_chainid
    ...
    await customRetryAsync(async function () {
      ...
      if (utils.isAddress(paymasterAddress)) {
        ...
        try {
          approveOp = await fuseMainNetSdk.estimate();
        } catch (e) {
          console.error(e);
          const eString = e.toString();
          addContext(test, eString);
        ...
        // get the uoHash1
        try {
          await fuseMainNetSdk.send(approveOp);
        } catch (e) {
          console.error(e);
          const eString = e.toString();
          addContext(test, eString);
